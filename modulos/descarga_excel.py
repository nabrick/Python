...
def exportar_excel(df, nombre_archivo='Archivo.xlsx', nombre_hoja='Reporte'):
    """
    Exporta un DataFrame a Excel.
    Si el archivo existe, crea uno nuevo agregando _1, _2, etc.
    Divide el DataFrame en varias hojas si supera max_filas.
    """
    ...
    with pd.ExcelWriter(nombre_archivo, engine='openpyxl', mode='w') as writer:
            ...
            for i in range(num_hojas):
                ...
                nombre_hoja_actual = f"{nombre_hoja}_{i+1}" if num_hojas > 1 else nombre_hoja
                df_parcial.to_excel(writer, sheet_name=nombre_hoja_actual, index=False)
